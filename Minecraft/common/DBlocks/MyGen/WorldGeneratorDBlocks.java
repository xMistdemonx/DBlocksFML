...
//(Max y - min y) + min y
	public void generateSurface(World world, Random rand, int chunkX, int chunkZ)
	{
		for (int i = 0; i < 55; i++)
		{
			...
			new WorldGenMinable(DBlocksIDS.DarkStone_BLOCK_ID,  60).generate(world, rand, randPosX, randPosY, randPosZ);
		}
		for (int i = 0; i < 55; i++)
		{
			int randPosX = chunkX + rand.nextInt(16);
			...
			int randPosZ = chunkZ + rand.nextInt(16);
			new WorldGenMinable(DBlocksIDS.LightStone_BLOCK_ID, 60).generate(world, rand, randPosX, randPosY, randPosZ);
		}
		for (int i = 0; i < 30; i++)
		{
			int randPosX = chunkX + rand.nextInt(16);
